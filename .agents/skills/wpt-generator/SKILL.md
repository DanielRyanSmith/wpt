...
**Temporary Files Policy:** If you need to create any temporary files during research, prototyping, or debugging, you **MUST** place them exclusively inside a `.wpt-generator-tmp/` directory at the root of the repository. Do not scatter temporary files across the codebase.
...
3. **Expand Existing Matrices:** Read the logically matching file(s). Do not treat the blueprint's `<title>` as a strict mandate to create a new file with that exact name. If the existing file uses a data-driven testing approach (e.g., an array of `testCases`), you MUST NOT blindly append new `promise_test` blocks to the bottom of the file. You MUST inject your new permutations directly into the existing data array. This leverages existing setup/teardown logic and automatically cross-multiplies your new test coverage against existing scenarios.
...
- **CRITICAL RULE: Style Guides > Golden Examples:** Existing tests ("Golden Examples") often contain legacy code and violate current best practices. **You MUST prioritize the explicit rules in the style guides over the paradigms found in surrounding files.** Do not blindly copy outdated instantiation patterns (e.g., manual `AbortController` setups instead of `AbortSignal.abort()`, unbounded polling instead of sentinels) or generic assertion patterns (e.g., `assert_true('key' in obj)` instead of `assert_own_property(obj, 'key')`). Use existing files to understand the *domain logic*, but rely exclusively on the style guides for the *implementation syntax*. For example, if the target directory tests 30 CSS property values using 30 flat `test_valid_value()` calls, you **MUST NOT** copy this. You must use a data-driven array and loop.
...
1. **Check for File:** Look for `WEB_FEATURES.yml` in the directory where you created or modified the test.
2. **Create if Missing:** If the file does not exist, create it with the following structure:
   ```yaml
   features:
   - name: <web_feature_id>
     files:
